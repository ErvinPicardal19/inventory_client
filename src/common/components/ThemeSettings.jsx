...
const ThemeSettings = () => {
  ...
  const MaterialUISwitch = styled(Switch)(({ theme }) => ({
    ...
    '& .MuiSwitch-thumb': {
      backgroundColor: currentMode === 'Dark' ? '#003892' : '#001e3c',
      width: 32,
      height: 32,
      '&:before': {
        content: "''",
    ...
    <div className='bg-half-transparent w-screen fixed nav-item top-0 right-0'>
      <div className='float-right h-screen dark:text-gray-200 bg-white dark:bg-main-dark-bg w-400'>
        ...
        <div className='flex-col border-t-1 border-color dark:border-white p-4 ml-4'>
          ...
          <FormGroup>
            <FormControlLabel
              control={<MaterialUISwitch sx={{ m: 1 }} label={currentMode} onChange={setMode} value={currentMode === 'Dark' ? 'Light' : 'Dark'} checked={currentMode === "Dark" ? true : false}/>}
              label={currentMode}
            />
          </FormGroup>
        </div>
            ...
            {themeColors.map((item, index) => (
              <TooltipComponent key={index} content={item.name} position="TopCenter">
                <div className='relative mt-2 cursor-pointer flex gap-5 items-center'>
                  ...
                  >
                    <BsCheck className={`ml-2 text-2xl text-white bg-half-transparent rounded-full ${item.color === currentColor ? 'block' : 'hidden'}`}/>
                  </button>
                </div>
              </TooltipComponent>
            ))}
